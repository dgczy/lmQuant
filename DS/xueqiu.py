...
(KHTML, like Gecko) Chrome/45.0.2454.101 Safari/537.36'
    ...
    def hist_price(code, start_date=None, end_date=None, period='D'):
        ...
        url = 'https://xueqiu.com/stock/forchartk/stocklist.json?symbol=%s&period=1day&type=normal' % (
            code)
        if not start_date is None:
        ...
        if len(ret_data) == 0:
            return None
        # 生成数据表
        df = pd.DataFrame(data=ret_data)[[
            'timestamp', 'open', 'close', 'high', 'low'
        ]]
        # 转换时间戳为标准日期格式
        df['timestamp'] = df['timestamp'].apply(
            lambda x: timestamp_to_date(x / 1000))
        # 重置索引
    ...
    def price(code, begin='', end='', days=0, timeout=5):
        ...
        if len(begin) > 0:
            url = '%s&begin=%d' % (
                ...
                int(
                    time.mktime(
                        datetime.strptime(begin, '%Y%m%d').timetuple()) *
                    1000))
        if len(end) > 0:
            url = '%s&end=%s' % (
                url,
                int(
                    time.mktime(datetime.strptime(end, '%Y%m%d').timetuple()) *
                    1000))
        if days > 0:
            url = '%s&begin=%d' % (
                url,
                int(
                    time.mktime(
                        (datetime.today() - timedelta(days)).timetuple()) *
                    1000))
        url = '%s&_=%d' % (url, int(time.time() * 1000))
        # 获取网站数据
        ...
        for item in stock_data:
            # 日期格式转换
            try:
                date = datetime.strptime(item['time'],
                                         "%a %b %d %H:%M:%S +0800 %Y")
            except:
                date = datetime.strptime(item['time'],
                                         "%a %b %d %H:%M:%S +0900 %Y")
