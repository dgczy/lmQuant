...
def get_stock_industry(code, date=pd.datetime.today()):
    try:
        return get_industry(security=code,
                            date=date)[code]['sw_l1']['industry_name']
    except:
        return None
...
def get_stock_valuation(code):
    ...
    return (round(df['pe_ratio'].iloc[0], 2), round(df['ps_ratio'].iloc[0], 2),
            round(df['pb_ratio'].iloc[0],
                  2), round(df['circulating_market_cap'].iloc[0], 2))
...
def get_stock_indicator(code, stat_year=2018, max_year=5, calc_year=5):
    ...
    net_profit = df['inc_net_profit_to_shareholders_year_on_year'][
        0:calc_year].mean()
    ...
    return (round(roe, 2), round(roe_mean, 2), round(gross_profit, 2),
            round(inc_revenue, 2), round(net_profit, 2), round(new_profit,
                                                               2), df)
...
def get_stock_goodwill(code, stat_date):
    ...
    if len(df) > 0:
        ...
        return (round(net_profit / 10000000,
                      4), round(net_assets / 100000000,
                                4), round(good_will / 100000000, 4),
                round(good_will / net_assets * 100, 4))
    else:
...
def get_bank_stock_indicator(code, date=2018):
    ...
    ).filter(bank_indicator.code == code)
    df = pd.DataFrame()
    #获取近五年财务信息
    for i in range(0, 5):
...
class Fields(TField):
    label = {
        ...
        'revenue_grow': '营复增(%)',
        'profit_grow': '利复增(%)',
        'new_profit_grow': '最新利增(%)',
        'net_profit': '净利润(千万)',
        'net_assets': '净资产(亿)',
        'good_will': '商誉(亿)',
        'good_will_ratio': '商誉占比(%)',
        'industry': '所属行业',
        'ipo_date': 'IPO日期'
    }
        ...
        if len(df) > 0:
            ...
            return (round(df.pe_ratio.iloc[-1],
                          2), round(df.pb_ratio.iloc[-1],
                                    2), round(df.ps_ratio.iloc[-1], 2),
                    round(df.pcf_ratio.iloc[-1], 2),
                    round(df.pb_ratio.iloc[-1] / df.pe_ratio.iloc[-1] * 100,
                          2), round(dyr, 2), round(cap, 2))
        else:
...
class _Value(Tvalue):
    ...
    def extend_analysis(self, code, df):
        ...
        (roe, roe_mean, Profit_margin, income, profit, new_profit,
         tdf) = get_stock_indicator(code)
        # 流通市值、股息、股息（均）
...
class Stk(object):
    ...
    def __init__(self,
                 ...
                 project_id=_ID,
                 project_note=_NOTE):
        self.project = TProject(data_name, data_path, project_name, project_id,
                                project_note)
        # 组合其它类
        ...
        self.change.chart = _Chart(self.project, self.pool, self.data,
                                   self.change)
        self.value.chart = _Chart(self.project, self.pool, self.data,
                                  self.value)
        self.change.table = _Table(self.project, self.pool, self.data,
                                   self.change)
        self.value.table = _Table(self.project, self.pool, self.data,
                                  self.value)
