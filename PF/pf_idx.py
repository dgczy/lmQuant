...
class _Pool(TPool):
    ...
    def create(self, pool):
        # 字典转换为dt
        df = pd.DataFrame(
            pool, columns=['code', 'name', 'market', 'type', 'watch', 'fund'])
        df.index = [
            df.code[i] + self.__convert_code(df.market[i])
    ...
    def to_track(self, df):
        ...
        # 估值标的
        self.value = df[df.market.isin(['sh', 'sz', 'hk'])].append(
            df[(df.market.isin(['us']) & df.watch)]).name.to_dict()
        # A股
        ...
        # 海外指数
        self.hw = df[df.market.isin(['hk', 'us', 'eu', 'as',
                                     'qt'])].name.to_dict()
        # 港股指数
...
class _Data(Tdata):
    ...
    def __get_hs_day(self, code, end_date):
        ...
        pb_w = cmc / (df['circulating_market_cap'] /
    ...
    def __get_hs(self, code, start_date=None, end_date=None):
        ...
        valuation_df = pd.DataFrame(data=valuation_list,
                                    index=date_list,
                                    columns=Fields.val + Fields.finance)
        # 拼接行情和估值数据
